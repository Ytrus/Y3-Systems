...
int init()

  {
   ...
   buyOpened = true; //all'inizio non voglio che entri quando metto il bot sul grafico. Fingo di avere già un ordine attivo
   sellOpened = true; //all'inizio non voglio che entri quando metto il bot sul grafico. Fingo di avere già un ordine attivo
...
{ 
   ...
   if (iBarShift(nomIndice,0,lastAnalizedBarTime,false) > 0 ) 
   {
      ...
      //dico al sistema che c'è una nuova barra: deve aprire un buy
      buyOpened = false;
...
//-----------------enter buy order---------------------------+
   ...
   if (buyOpened == false)                                     buyConditions[1] = true; // non ho già apero un ordine in questa barra
   if (MarketInfo(nomIndice,MODE_BID) >= Low[1])               buyConditions[2] = true; // se il prezzo sta scendendo non devo rientrare (mi ha già buttato fuori quando ho toccato min)
   if (!existOpendedAndClosedOnThisBar(2))                     buyConditions[3] = true; // Se non ho 2 ordini aperti e chiusi in questa barra
   if (Low[0] > min)                                           buyConditions[4] = true; // solo se la barra attuale non è anche il minimo di giornata 
   ...
   if (existOrder(0) < 0)                                      buyConditions[6] = true; // non ho già un ordine aperto in questa direzione (apre un solo ordine per direzione)
...
//-----------------enter sell order----------------------------+
   ...
   if (MarketInfo(nomIndice,MODE_BID) <= High[1])              sellConditions[1] = true; // se il prezzo sta salendo non devo rientrare (mi ha già buttato fuori quando ho toccato max)
