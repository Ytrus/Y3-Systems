...
//--------------------------+
string bot_name = "ADX 0.2.0"; //Sistema ADX ger30 1M
...
extern string COMMENT = "ADX";
extern string nameOfHistoryFile = "ADX_HST_";
...
double STR_shift = 2; // per compatibilità con la libreria
...
double plusDI[3], minusDI[3], ADX[3];
...
{ 
   ...
   for (int i=1;i<3;i++)
   {  
      ADX[i]  = iADX(nomIndice,0,14,PRICE_CLOSE,0,i);
      plusDI[i]  = iADX(nomIndice,0,14,PRICE_CLOSE,1,i);
      minusDI[i] = iADX(nomIndice,0,14,PRICE_CLOSE,2,i);
   }   
//-----------------enter buy order---------------------------+

   // buyConditions array
   
   if (plusDI[1]>minusDI[1])                                   buyConditions[0] = true;   // +DI è salito sopra a -DI
   if (plusDI[2]<minusDI[2])                                   buyConditions[1] = true;   // ma prima era inferriore
   if (Close[0]>Close[1])                                      buyConditions[2] = true;   // il prezzo supera quello della barra precedente
   if (enabledHours[Hour()] == true)                           buyConditions[3] = true;   // A questa ora posso tradare
   if (!existOpendedAndClosedOnThisBar(1))                     buyConditions[4] = true;   // Se non ho 1 ordine aperto e chiuso in questa barra
   if (existOrder(0) < 0)                                      buyConditions[5] = true;   // non ho già un ordine aperto in questa direzione (apre un solo ordine per direzione)
...
for(int pos=0;pos<OrdersTotal();pos++)
    {
     ...
     if ((ADXturned("BUY"))                                                                        // L'ADX si gira
       //(isCameBack_One(OrderTicket(), 0))                                                        // se dopo aver visto un profitto di 1:1 torna indietro
...
//-----------------enter sell order----------------------------+
   ...
   if (plusDI[1]<minusDI[1])                                   sellConditions[0] = true;   // +DI è salito sopra a -DI
   if (plusDI[2]>minusDI[2])                                   sellConditions[1] = true;   // ma prima era inferriore
   if (Close[0]<Close[1])                                      sellConditions[2] = true;   // il prezzo supera quello della barra precedente
   if (!existOrderOnThisBar(1))                                sellConditions[2] = true;   // se NON ho un ordine già aperto in questa barra (apre più ordini in ogni direzione)
   if (enabledHours[Hour()] == true)                           sellConditions[3] = true;   // A questa ora posso tradare
   if (!existOpendedAndClosedOnThisBar(1))                     sellConditions[4] = true;   // Se non ho 1 ordine aperto e chiuso in questa barra   
   if (existOrder(1) < 0 )                                     sellConditions[5] = true;     // non ho già un ordine attivo in questa direzione (apre un solo ordine per direzione)
...
for(int pos=0;pos<OrdersTotal();pos++)
    {
     ...
     //clausole di chiusura
     if ((ADXturned("SELL"))                                                                         // L'ADX si gira
       // (isCameBack_One(OrderTicket(), 0))                                                         // se dopo aver visto un profitto di 1:1 torna indietro
       || (protector(OrderTicket(), protectionStartDistance, protectionCloseDistance))               // se ha raggiunto una certa percentuale di profitto e poi torna indietro
       //|| (slReached(OrderTicket()))                                                               // Raggiunto SL
       || (tpReached(OrderTicket()))                                                               // Raggiunto TP
