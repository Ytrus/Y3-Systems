...
extern int atrTP = 7;
extern int OrderDistance = 5;                     /*Order Distance*/
extern int maxOrders = 5;                        /*Max number of Orders*/
...
{ 
   ...
   if (iBarShift(nomIndice,0,lastAnalizedBarTime,false) > 0 ) //(Volume[0] == 1)
   {
      ...
      // Calcolo ADX
      for(int i = 0; i<10; i++){
         adx[i] = iADX(nomIndice,PERIOD_CURRENT,14,PRICE_CLOSE,MODE_MAIN,i);
         plusDI[i] = iADX(nomIndice,PERIOD_CURRENT,14,PRICE_CLOSE,MODE_PLUSDI,i);
         minusDI[i] = iADX(nomIndice,PERIOD_CURRENT,14,PRICE_CLOSE,MODE_MINUSDI,i);
         //maH4[i] = iMA(nomIndice,PERIOD_H4,200,0,MODE_SMMA,PRICE_TYPICAL,i);
      }

      for(int i = 0; i<30; i++){
         maCurrent[i] = iMA(nomIndice,PERIOD_CURRENT,800,0,MODE_SMMA,PRICE_TYPICAL,i);
      }      
   }
...
//-----------------enter buy order---------------------------+
   ...
   if (adx[1] < adx[2])                                        buyConditions[0] = true;   // Se adx si è girato
   if (adx[2] > adx[3])                                        buyConditions[1] = true;   // rispetto alla barra precedente
   if (adx[3] > adx[4])                                        buyConditions[2] = true;   // e anche quella prima era in salita
   ...
   if (enabledHours[Hour()] == true)                           buyConditions[4] = true;   // A questa ora posso tradare
   ...
   if (!existOpendedAndClosedOnThisBar(1))                     buyConditions[6] = true;   // Se non ho 1 ordine aperto e chiuso in questa barra
...
for(int pos=0;pos<OrdersTotal();pos++)
    {
     ...
     if ((haveMinimumProfit("BUY"))                                                          // se ho il profitto minimo richiesto in caso di ordini multipli aperti
     //|| (moreThanOneOrderOnBrakeEven("BUY"))                                                 // se ho più di un ordine mi accontento del BreakEven appena lo vedo
     //|| closeAfterTooMuchTime("BUY")
     //|| (maIsWrong("BUY"))                                                                   // Se la media si è girata
...
//-----------------enter sell order----------------------------+
   ...
   if (adx[1] < adx[2])                                        sellConditions[0] = true;   // Se adx si è girato
   if (adx[2] > adx[3])                                        sellConditions[1] = true;   // rispetto alla barra precedente
   if (adx[3] < adx[4])                                        sellConditions[2] = true;   // e anche quella prima era in discesa
   if (!existOrderOnThisBar(1))                                sellConditions[3] = true;   // se NON ho un ordine già aperto in questa barra (apre un solo ordine per ogni direzione)
   if (enabledHours[Hour()] == true)                           sellConditions[4] = true;   // A questa ora posso tradare
   ...
   if (!existOpendedAndClosedOnThisBar(1))                     sellConditions[6] = true;   // Se non ho 1 ordine aperto e chiuso in questa barra
...
for(int pos=0;pos<OrdersTotal();pos++)
    {
     ...
     //clausole di chiusura
     if ((haveMinimumProfit("SELL"))                                                         // se ho il profitto minimo richiesto in caso di ordini multipli aperti
     //|| (moreThanOneOrderOnBrakeEven("SELL"))                                                // se ho più di un ordine mi accontento del BreakEven appena lo vedo
     //|| closeAfterTooMuchTime("SELL")
       //|| (maIsWrong("SELL"))                                                                // se la media si è girata
       //|| (protector(OrderTicket(), protectionStartDistance, protectionCloseDistance))   // se ha raggiunto una certa percentuale di profitto e poi torna indietro
       //|| (tpReached(OrderTicket()))                                                       // Raggiunto TP
       //|| (slReached(OrderTicket())                                                         // Raggiunto SL
       )
   ...
   for(int pos=0;pos<total;pos++)
   {
      ...
      if ( (OrderMagicNumber() == SIGNATURE) && (OrderSymbol() == nomIndice) && (OrderCloseTime() == 0) && (OrderType() == OP_BUY))  {       
         ...
         while (!t){
            ...
            if (!t) { Print("Errore nella chiusura di un orine Buy: "+GetLastError()); RefreshRates(); }
         }
   ...
   for(int pos=0;pos<total;pos++)
   {
      ...
      if ( (OrderMagicNumber() == SIGNATURE) && (OrderSymbol() == nomIndice) && (OrderCloseTime() == 0) && (OrderType() == OP_SELL))  {       
         ...
         while (!t){
            ...
            if (!t) { Print("Errore nella chiusura di un orine Sell: "+GetLastError()); RefreshRates();break;}
         }
...
// =====================================================================
// se ho più ordini chiudo appena vedo un profitto, anche minimo
// =====================================================================
bool moreThanOneOrderOnBrakeEven(string dir){
   ...
   if(ot == OP_BUY){
      ...
      // genero la media del prezzo di acquisto
      if (orders > 1) {
         middlePrice = NormalizeDouble(middlePrice/orders, Digits);
      
         targetPrice = middlePrice;
         // sposto la riga della media buy
         if(!ObjectMove("Buy Average",0,0,targetPrice)) Print("Fallito lo spostamento della media Buy");
...
// =================================================
// se si gira la media, esco
// =================================================
bool maIsWrong(string dir){
   bool result = false;
   
   if (dir == "BUY"){
      if ((maCurrent[0] < maCurrent[9]) && (maCurrent[10] < maCurrent[20]) ) result = true;
   }
...
// =================================================
// Se un ordine non va a profitto entro x barre (primi test su 350) lo chiudo
// =================================================
bool closeAfterTooMuchTime(string dir){
   int total = OrdersTotal();
   int orders = 0;
   ...
   {     
      if(OrderSelect(pos,SELECT_BY_POS)==false) continue;
      if ( (OrderMagicNumber() == SIGNATURE) && (OrderSymbol() == nomIndice) && (OrderCloseTime() == 0) && (OrderType() == ot) && (iBarShift(nomIndice,PERIOD_CURRENT,OrderOpenTime(),false) > 1000)){
          orders++; // conto gli ordini
          tkt = OrderTicket();
          lots = OrderLots();
          if(ot==OP_BUY) priceClose = Bid;
          if(ot==OP_SELL) priceClose = Ask;
      
         // ----- continuo a tentare di chiudere l'ordine all'infinito, finchè ci riesco
         while (!t) {
            t = OrderClose(tkt,lots,priceClose,5,Brown);
