...
extern int SIGNATURE = 0019100;
extern string COMMENT = "SEB";
extern string nameOfHistoryFile = "SEB_System_HST_";
...
double haOpen[5], haClose[5], haHigh[5], haLow[5]; // arrays con i valori delle Haiken Ashi
...
bool closeOrders = false; // all'inizio di ogni barra la metto a true per chiudere gli ordini aperti
...
{ 
   ...
   // ====================================================
   // Operazioni eseguite una sola volta per ogni barra   
   // ====================================================
   if (iBarShift(nomIndice,0,lastAnalizedBarTime,false) > 0 ) 
   {
      ...
      closeOrders = true; // dico al sistema di chiuedere eventuali ordini aperti
...
//-----------------enter buy order---------------------------+
   ...
   if (haClose[0] > haOpen[0])                                 buyConditions[2] = true; // la barra HA attuale è BULL
   ...
   if (MarketInfo(nomIndice,MODE_BID) >= Low[1])               buyConditions[5] = true; // se il prezzo sta scendendo non devo rientrare (mi ha già buttato fuori quando ho toccato min)
   if (Low[0] > min)                                           buyConditions[6] = true; // solo se la barra attuale non è anche il minimo di giornata 
   if (existOrder(0) < 0)                                      buyConditions[7] = true; // non ho già un ordine aperto in questa direzione (apre un solo ordine per direzione)
...
// TIP: per usare SL e TP fissi, basta fissarli a 1000 Points in più di quello che dovrebbero essere e poi chiuderli qui quando li raggiungono con 100 Points di differenza.
...
for(int pos=0;pos<OrdersTotal();pos++)
    {
     ...
     if ( (isOrderOnPrevBar(OrderTicket()))
//       (haClose[1] < haOpen[1])                                           // la barra precedente è BEAR
...
//-----------------enter sell order----------------------------+
   ...
   if (haClose[0] < haOpen[0])                                 sellConditions[2] = true; // la barra HA attuale è BEAR
   ...
   if (MarketInfo(nomIndice,MODE_BID) <= High[1])              sellConditions[5] = true; // se il prezzo sta salendo non devo rientrare (mi ha già buttato fuori quando ho toccato max)
   if (High[0] < max)                                          sellConditions[6] = true; // solo se la barra attuale non è anche il massimo di giornata 
   if (existOrder(1) < 0 )                                     sellConditions[7] = true;// non ho già un ordine attivo in questa direzione (apre un solo ordine per direzione)
...
for(int pos=0;pos<OrdersTotal();pos++)
    {
     ...
     //clausole di chiusura
     if (   (profitIsXPips(OrderTicket(), 30))
         || (isOrderOnPrevBar(OrderTicket()))
//          (haClose[1] > haOpen[1])                             // la barra precedente è BULL
//          (closeSell)                                          // quando apre una nuova barra, chiudo il vecchio ordine
       || (slReached(OrderTicket()))                           // Raggiunto SL
//       || (tpReached(OrderTicket()))                           // Raggiunto TP
       )
     {
...

{
   ...
   for (int orx=1;orx<=numberOfOrders;orx++)
   {
      ...
      {  
         ...
         stoploss   = (MarketInfo(nomIndice,MODE_ASK) - (SL_added_pips*Point));
...

{
   ...
   for (int orx=1;orx<=numberOfOrders;orx++)
   {
   ...
   
      {
         ...
         stoploss   = (MarketInfo(nomIndice,MODE_BID) + (SL_added_pips*Point));
...
//------- Guarda se l'ordine è stato aperto nella barra precedente ----------+

bool isOrderOnPrevBar(int ot) 
   {
      bool result = false;
      
      if (OrderSelect(ot,SELECT_BY_TICKET)==true)
      {
         if ( (OrderMagicNumber() == SIGNATURE) && (OrderSymbol() == nomIndice) && (OrderCloseTime() == 0) && (iBarShift(nomIndice,0,OrderOpenTime(),false) == 1) )         
         {return true;}
...
bool profitIsXPips(int ot, int ticks) 
   {
      bool result = false;
      ...
      if (OrderSelect(ot,SELECT_BY_TICKET)==true)
      {
         ...
         if ( (OrderMagicNumber() == SIGNATURE) && (OrderSymbol() == nomIndice) )         
         {
            // per ora guardo solo gli ordini sell
            double gainedPips = ( OrderOpenPrice()-MarketInfo(nomIndice, MODE_ASK) ) / Point; // se sta guadagnando X pips, restituisco true
            
            if (gainedPips >= ticks) return true;
         }
