...
//--------------------------+
string bot_name = "AMA Inverted 0.1.0"; // Per il TP e lo SL uso l'ATR orario (2x e 1x), poi muovo lo SL al salire del profitto
...
double AMAslowUP[10], AMAslowDOWN[10], AMAfastUP[10], AMAfastDOWN[10];
...
// ------------------ Attribuzione Haiken Ashi --------------------
   ...
   {  
      ...
      //PROVA CON FILTRO AMA
      
      AMAslowUP[i] = iCustom(nomIndice,slowerPeriod,"downloads\\AMA",9,2,30,2.0,2.0,1,i);
      AMAslowDOWN[i] = iCustom(nomIndice,slowerPeriod,"downloads\\AMA",9,2,30,2.0,2.0,2,i);
      AMAfastUP[i] = iCustom(nomIndice,PERIOD_CURRENT,"downloads\\AMA",9,2,30,2.0,2.0,1,i);
      AMAfastDOWN[i] = iCustom(nomIndice,PERIOD_CURRENT,"downloads\\AMA",9,2,30,2.0,2.0,2,i);
      
   }
    //PROVA CON FILTRO AMA
    double trendUp, trendDown;
    trendUp = iCustom(nomIndice,slowerPeriod,"downloads\\AMA",9,2,30,2.0,2.0,1,0);
    trendDown = iCustom(nomIndice,slowerPeriod,"downloads\\AMA",9,2,30,2.0,2.0,2,0);
...
//-----------------enter buy order---------------------------+
   ...
   if (AMAslowDOWN[0] > 0)                                     buyConditions[0] = true;   // il trend superiore è DOWN
   if ((AMAfastDOWN[1] == 0) && (AMAfastDOWN[0] > 0) )         buyConditions[1] = true;   // nasce il pallino rosso nel Timeframe attuale
   if (!existOrderOnThisBar(0))                                buyConditions[2] = true;   // se NON ho un ordine già aperto in questa barra (apre più ordini in ogni direzione)
...
//-----------------enter sell order----------------------------+
   ...
   if (AMAslowUP[0] > 0)                                       sellConditions[0] = true;   // il trend superiore è UP
   if ((AMAfastUP[1] == 0) && (AMAfastUP[0] > 0) )             sellConditions[1] = true;   // nasce il pallino blu nel Timeframe attuale
...
double getTP(string direction = "BUY"){
   ...
   double atrValue = iATR(nomIndice,PERIOD_CURRENT, 14, 0); //atr 14 orario
   double result = 0;
   if (direction == "BUY"){
...
bool isCameBack_One(int tkt, int protectionPercent)
{
   ...
   if (OrderSelect(tkt, SELECT_BY_TICKET)==true) 
   {
      ...
      if ((OrderType() == OP_SELL) && (shift > 0)) // buy order
      {
         max_ = High[iHighest(nomIndice,0,MODE_HIGH,shift,0)]; //Print("isCameBack BUY: profit="+profit+" -- max_="+max_+" -- shift="+shift);
      ...
      if ((OrderType() == OP_BUY) && (shift > 0) ) // sell order
      {
         min_ = Low[iLowest(nomIndice,0,MODE_LOW,shift,0)]; //Print("isCameBack SELL: profit="+profit+" -- min_="+min_+" -- shift="+shift);
