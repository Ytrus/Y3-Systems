...
string bot_name = "STR M1";
string nomIndice = "GER30"; //sovrascritto dopo in init()

extern int SIGNATURE = 0037000;
extern string COMMENT = "STR M1";
...
*/
   ...
   for(int pos=0;pos<OrdersTotal();pos++)
   {
      ...
      if ( 
         ( (prevMinIsBroken(OrderOpenTime())) )                                                        // se supero il minimo precedente
...
*/
   ...
   for(int pos=0;pos<OrdersTotal();pos++)
   {
      ...
      // chiusura SINGOLO ordine 
      if (
         ( (prevMaxIsBroken(OrderOpenTime()))  )                                                      // se supero il massimo precedente
         || (protector(OrderTicket(), protectionStartDistance, protectionCloseDistance))            // se ha raggiunto una certa percentuale di profitto e poi torna indietro
      ) closeOrder(OrderTicket());
...
//--------------------------------------------------------+
//            rotto il minimo precedente 
//--------------------------------------------------------+
bool prevMinIsBroken (datetime t){
   bool result = false;
   int openBarShift = iBarShift(nomIndice,PERIOD_CURRENT,t,false)+1; // barra in cui l'ordine è stato aperto (+1 perchè al momeno apro gli ordini al primo tick della barra successiva)
   if (openBarShift < 1) return false;
   int lowestBarShift = iLowest(nomIndice,PERIOD_CURRENT,MODE_LOW,openBarShift,1);
   
   if( (Low[lowestBarShift-1] > Low[lowestBarShift]) && (Close[0] < Low[lowestBarShift]) )   result = true;
...
bool prevMaxIsBroken (datetime t){
   ...
   int openBarShift = iBarShift(nomIndice,PERIOD_CURRENT,t,false)+1; // barra in cui l'ordine è stato aperto (+1 perchè al momeno apro gli ordini al primo tick della barra successiva)
   if (openBarShift < 1) return false;
   int highestBarShift = iHighest(nomIndice,PERIOD_CURRENT,MODE_HIGH,openBarShift,1);
   
   if( (High[highestBarShift-1] < High[highestBarShift]) && (Close[0] > High[highestBarShift]) )   result = true;
