...
//--------------------------+
string bot_name = "HA System 0.3.5 - AMA"; // Utilizza la libreria supertrend
...
extern bool enableClassicTP = true;             /*Classic TP*/
...
int init()

  {
   ...
   botSettings += "Super SL:+"+(string)enableSuperSL+",+";
...
bool SuperSLReached(int tkt){
   ...
   if (OrderSelect(tkt, SELECT_BY_TICKET)==true) 
   {
      if ((OrderType() == OP_BUY) ) // buy order
      {
         ...
         if (MarketInfo(nomIndice,MODE_BID) < superSL) return true;
      }
