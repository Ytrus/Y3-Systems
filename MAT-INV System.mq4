...
// NOTE
// comprare sulla rottura rialzista di ogni barra quando il prezzo si trova sopra la media mobile
// vendere sulla rottura ribassista di ogni barra quando il prezzo si trova sotto la media mobile
// tp1 pari al rischio, tp2 2*rischio, tp3 3*rischio
//
...
extern string nameOfHistoryFile = "MAT_System_HST_";
...
//-----------------enter buy order---------------------------+
   ...
   if (High[1] > fastMA)                                       buyConditions[1] = true; // la barra di ieri ha il max sopra alla media mobile
   if (MarketInfo(nomIndice,MODE_BID) > High[1]+3*Point)       buyConditions[2] = true; // il prezzo rompe il massimo della barra precedente...
   if (MarketInfo(nomIndice,MODE_BID) < High[1]+6*Point)       buyConditions[3] = true; // ...ma non più di 3 punti
   //if (Close[1] > fastMA)                                    buyConditions[4] = true; // la chiusura della barra precedente è sopra la media mobile
   if (High[0] < High[1]+20*Point)                             buyConditions[5] = true; // il massimo di oggi non è oltre xx pips del massimo della barra di ieri. Significa che sono già entrato ed uscito!
...
//      && (buyConditions[4])  
   ...
   )
   {
         ...
         entreeSell[0] = true;// entreeBuy[0] = true;
         //entreeBuy[1] = true;
         //entreeBuy[2] = true;
         //Print("BUY - b:",b," - a:",a);
   }
...
//-----------------exit buy orders---------------------------+
   ...
   if (OrderSelect(ticketSell[0], SELECT_BY_TICKET)==true) 
   {
      ...
      if ( (MarketInfo(nomIndice,MODE_BID) > High[shift]+profit+4*Point)  // se vedo un profitto pari al  rischio, chiudo il primo ordine
          || (MarketInfo(nomIndice,MODE_BID)<fastMA+4*Point)              // se il prezzo scende sotto la media mobile veloce
          || (MarketInfo(nomIndice,MODE_BID)<Low[shift]-1*Point)  // se il prezzo scende sotto al minimo della pin
         )
         
      {      
          sortieSell[0] = true; //sortieBuy[0] = true;
      }
...
//-----------------enter sell order----------------------------+
   ...
   if (Low[1] < fastMA)                                       sellConditions[1] = true; // la barra di ieri ha il max sopra alla media mobile
   if (MarketInfo(nomIndice,MODE_BID) < Low[1]-3*Point)        sellConditions[2] = true; // il prezzo rompe il minimo della pin...
   if (MarketInfo(nomIndice,MODE_BID) > Low[1]-6*Point)        sellConditions[3] = true; // ... ma non più di 6 punti
   //if (Close[1] < fastMA)                                      sellConditions[4] = true; // il minimo della pin è sotto la media mobile
   if (Low[0]>Low[1]-20*Point)                                 sellConditions[5] = true; // il minimo di oggi è oltre 10 pips sotto alla pin. Significa che sono già entrato ed uscito!
...
//      && (sellConditions[4]) 
   ...
   {
      // attivo tutti gli ordini
      entreeBuy[0] = true; // entreeSell[0] = true; 
      //entreeSell[1] = true; 
      //entreeSell[2] = true; 
      //Print("SELL - b:",b," - a:",a);
   }
...
//-----------------exit sell order ---------------------------+
   ...
   if (OrderSelect(ticketBuy[0], SELECT_BY_TICKET)==true) 
   {
      ...
      if (
            (MarketInfo(nomIndice,MODE_BID) < Low[shift]-profit-4*Point)  // se vedo un profitto pari al rischio, chiudo
            || (MarketInfo(nomIndice,MODE_BID) > fastMA-4*Point ) // se il prezzo sale sopra alla media mobile veloce
            || (MarketInfo(nomIndice,MODE_BID) > High[shift]+1*Point ) // se il prezzo sale sopra al massimo della pin
         
         )
      {
       sortieBuy[0] = true; // sortieSell[0] = true;
      }
...

{
   ...
   if(tradeBuy[0] == false && tradeSell[0] == false && entreeBuy[0] == true)
   {
      ...
      ticketBuy[0] = OrderSend(nomIndice,OP_BUY,setPower(POWER),MarketInfo(nomIndice,MODE_ASK),8,stoploss,takeprofit,"MAT_System" ,SIGNATURE,0,MediumBlue);
   ...
   if(tradeBuy[1] == false && tradeSell[1] == false && entreeBuy[1] == true)
   {
      ...
      ticketBuy[1] = OrderSend(nomIndice,OP_BUY,setPower(POWER),MarketInfo(nomIndice,MODE_ASK),8,stoploss,takeprofit,"MAT_System" ,SIGNATURE,0,MediumBlue);
   ...
   if(tradeBuy[2] == false && tradeSell[2] == false && entreeBuy[2] == true)
   {
      ...
      ticketBuy[2] = OrderSend(nomIndice,OP_BUY,setPower(POWER),MarketInfo(nomIndice,MODE_ASK),8,stoploss,takeprofit,"MAT_System" ,SIGNATURE,0,MediumBlue);
...

{
...

   {
      ...
      ticketSell[0] = OrderSend(nomIndice,OP_SELL,setPower(POWER),MarketInfo(nomIndice,MODE_BID),8,stoploss,takeprofit,"MAT_System" ,SIGNATURE,0,MediumBlue);
...

   {
      ...
      ticketSell[1] = OrderSend(nomIndice,OP_SELL,setPower(POWER),MarketInfo(nomIndice,MODE_BID),8,stoploss,takeprofit,"MAT_System" ,SIGNATURE,0,MediumBlue);
...

   {
      ...
      ticketSell[2] = OrderSend(nomIndice,OP_SELL,setPower(POWER),MarketInfo(nomIndice,MODE_BID),8,stoploss,takeprofit,"MAT_System" ,SIGNATURE,0,MediumBlue);
