...
{
//---
   atr = iATR(nomIndice,PERIOD_CURRENT,100,1);
...
bool isDownSignal(){
   ...
   // TODO: aggiungere qui sopra anche le condizioni sulla dimensione delle barre (vola)
   //Print("*************   lastBarSize:"+lastBarSize+"  - atr:"+atr+"    *************");
   return result;
...
bool isUpAMA(){
   ...
   int counter = 0;
   double AMAtwo   = iCustom(nomIndice,0,"Downloads\\AMA",9,2,30,2,2,1,2);
   double AMAone   = iCustom(nomIndice,0,"Downloads\\AMA",9,2,30,2,2,1,1);
   double AMAzero  = iCustom(nomIndice,0,"Downloads\\AMA",9,2,30,2,2,1,0);
   double lastdegree = MathAbs(AMAzero-AMAone);
   bool result = false;
   
   
   if (AMAzero > 0)           counter++;
   if (AMAone > 0)            counter++;
   if (AMAtwo > 0)            counter++;
   if (lastdegree >= atr*minAMADegree)  counter++;
   //counter++;
...
bool isDownAMA(){
   if (!useAMA_Filter) return true;
   int counter = 0;
   double AMAtwo   = iCustom(nomIndice,0,"Downloads\\AMA",9,2,30,2,2,2,2);
   double AMAone   = iCustom(nomIndice,0,"Downloads\\AMA",9,2,30,2,2,2,1);
   double AMAzero  = iCustom(nomIndice,0,"Downloads\\AMA",9,2,30,2,2,2,0);
   double lastdegree = MathAbs(AMAzero-AMAone);
   bool result = false;
...
bool stoppedByAMA(int ot){
   ...
   double AMAone = 0;
   if(ot==OP_BUY)  {AMAone = iCustom(nomIndice,0,"Downloads\\AMA",9,2,30,2,2,2,1);}
   if(ot==OP_SELL) {AMAone = iCustom(nomIndice,0,"Downloads\\AMA",9,2,30,2,2,1,1);}
   
   bool result = false;
   
 
   if (AMAone > 0) result = true;
...
bool openOrder(int ot, double sz){
   ...
   if (ot == OP_BUY) {  prz = MarketInfo(nomIndice,MODE_ASK); 
                        sl  = Low[iLowest(nomIndice,PERIOD_CURRENT,MODE_LOW,1,1)]-spread*Point; 
                        tp  = NormalizeDouble(prz+tpMultiplier*MathAbs(prz-sl),Digits);  c = clrBlue;
                        //sl = prz - 1000*Point;
                        //tp = prz + 500*tpMultiplier*Point; 
                     }
   if (ot == OP_SELL){  prz = MarketInfo(nomIndice,MODE_BID); 
                        sl  = High[iHighest(nomIndice,PERIOD_CURRENT,MODE_HIGH,1,1)]+2*spread*Point; 
