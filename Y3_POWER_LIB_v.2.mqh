...
int addOrderToHistory(int ticket){
   ...
   int maxK = 52; //numero massimo di valori nell'array, determinato dal massimo periodo della ema sui profitti (per ver.7 è 52)
   int k = ArraySize(historicPips); //get array size
   //Print("addOrderToHistory - Array size: "+k);
   if (k<maxK) {
      ArrayResize(historicPips,k+1,1000); //increment it by one to put in the new profit or loss in pips
      ArrayResize(historicPipsMA,k+1,1000); //increment it by one to put in the new MA value
   }
   
   //dopo di che riprendo k come attuale dimensione dell'array meno uno, perchè uso k per storare l'ultimo valore nell'ultima posizione dell'array, più avanti
   k = ArraySize(historicPips)-1;
   ...
   // se ho raggiunto il massimo numero di valori nell'array (maxK) devo spostare i valori indietro di una posizione e non aggiungere quello nuovo
   if (k == maxK-1) {for (int i=1; i<=k; i++) historicPips[i-1] = historicPips[i];}
