...
extern int SIGNATURE = 0030000;
extern string COMMENT = "Super_M";
extern string nameOfHistoryFile = "Super_M_HST_";
...
extern double STR_shift = 2;                       /*Supertrend Lib Shift*/
extern bool enableSuperSL = true;                  /*Super Stop Loss*/
extern double superEntry_ratio = 0.3;              /*Super Entry Ratio*/ 
extern double superSL_ratio = 0.7;                 /*Super Stop Loss Ratio*/ 
extern int super_Period = 10;                    /*Super MA*/
extern bool enableClassicTP = false;               /*Classic TP*/
...
double superSL_MA[10];
double superSL_BuySignal[10];
double superSL_SellSignal[10];
double superSL_StopBuy[10];
double superSL_StopSell[10];
...
int init()

  {
   ...
   botSettings += "STR_shift:+"+(string)STR_shift+",+";
   botSettings += "enableSuperSL:+"+(string)enableSuperSL+",+";
   botSettings += "superEntry_ratio:+"+(string)superEntry_ratio+",+";
   botSettings += "super_Period:+"+(string)super_Period+",+";
   botSettings += "superSL_ratio:+"+(string)superSL_ratio+",+";
   botSettings += "enableClassicTP:+"+(string)enableClassicTP+",+";  
...
{ 
   ...
   if (iBarShift(nomIndice,0,lastAnalizedBarTime,false) > 0 ) //(Volume[0] == 1)
   {
      ...
      // =====================================
      for(int i=0; i<10; i++){
      superSL_MA[i] = iCustom(nomIndice,0,"Y3_SuperSL",super_Period,0,1,superEntry_ratio,superSL_ratio,0,i);
      superSL_BuySignal[i] = iCustom(nomIndice,0,"Y3_SuperSL",super_Period,0,1,superEntry_ratio,superSL_ratio,1,i);
      superSL_SellSignal[i] = iCustom(nomIndice,0,"Y3_SuperSL",super_Period,0,1,superEntry_ratio,superSL_ratio,2,i);
      superSL_StopBuy[i] = iCustom(nomIndice,0,"Y3_SuperSL",super_Period,0,1,superEntry_ratio,superSL_ratio,3,i);
      superSL_StopSell[i] = iCustom(nomIndice,0,"Y3_SuperSL",super_Period,0,1,superEntry_ratio,superSL_ratio,4,i);
      }      
...
//-----------------enter buy order---------------------------+
   ...
   if ( (superSL_MA[1]>superSL_MA[2]) && (superSL_MA[2]<superSL_MA[3]) )   buyConditions[0] = true;   // il prezzo si è girato la barra prcedente
   if ( (superSL_MA[2]>superSL_MA[3]) && (superSL_MA[3]<superSL_MA[4]) && (superSL_MA[1] > superSL_MA[2]) )   buyConditions[1] = true;   // oppure la barra prima ancora
   if ( (superSL_MA[3]>superSL_MA[4]) && (superSL_MA[4]<superSL_MA[5]) && (superSL_MA[1] > superSL_MA[2]) )   buyConditions[2] = true;   // oppure la barra prima ancora
   if (MarketInfo(nomIndice, MODE_BID)>superSL_StopBuy[0])                 buyConditions[3] = true;   // non siamo sotto allo SL
   if (MarketInfo(nomIndice, MODE_BID)<=superSL_BuySignal[0])              buyConditions[4] = true;   // il prezzo torna giu fino al livello di ingresso
   ...
   if(   ( (buyConditions[0]) || (sellConditions[1]) || (sellConditions[2]))
      && (buyConditions[3])
      && (buyConditions[4])
      && (buyConditions[5]) 
      && (buyConditions[6]) 
...
for(int pos=0;pos<OrdersTotal();pos++)
    {
     ...
     if (//(isCameBack_One(OrderTicket(), 0))                                                        // se dopo aver visto un profitto di 1:1 torna indietro
       ...
       (SuperSLReached(OrderTicket()))                                                            // viene superato il super Stop Loss
)
     {
...
//-----------------enter sell order----------------------------+
   ...
   if ( (superSL_MA[1]<superSL_MA[2]) && (superSL_MA[2]>superSL_MA[3]) )   sellConditions[0] = true;   // il prezzo si è girato la barra prcedente
   if ( (superSL_MA[2]<superSL_MA[3]) && (superSL_MA[3]>superSL_MA[4]) && (superSL_MA[1] < superSL_MA[2]) )   sellConditions[1] = true;   // oppure la barra prima ancora
   if ( (superSL_MA[3]<superSL_MA[4]) && (superSL_MA[4]>superSL_MA[5]) && (superSL_MA[1] < superSL_MA[2]) )   sellConditions[2] = true;   // oppure la barra prima ancora
   if (MarketInfo(nomIndice, MODE_BID)<superSL_StopSell[0])                sellConditions[3] = true;   // non siamo sopra allo SL
   if (MarketInfo(nomIndice, MODE_BID)>=superSL_SellSignal[0])             sellConditions[4] = true;   // il prezzo torna su fino al livello di ingresso
   ...
   if(   ((sellConditions[0]) || (sellConditions[1]) || (sellConditions[2]) )
      && (sellConditions[3])
      && (sellConditions[4])
      && (sellConditions[5]) 
      && (sellConditions[6]) 
...

{
   ...
   for (int orx=1;orx<=numberOfOrders;orx++)
   {
      ...
      {  
         ...
         //senza libreria
         ticketBuy = OrderSend(nomIndice,OP_BUY,size,MarketInfo(nomIndice,MODE_ASK),8,stoploss,takeprofit,COMMENT ,SIGNATURE,0,MediumBlue);
...

{
   ...
   for (int orx=1;orx<=numberOfOrders;orx++)
   {
   ...
   
      {
         ...
         // senza libreria
         ticketSell = OrderSend(nomIndice,OP_SELL,size,MarketInfo(nomIndice,MODE_BID),8,stoploss,takeprofit,COMMENT ,SIGNATURE,0,Purple);
...
// =======================================
//         Super SL indicator - SL
// =======================================
bool SuperSLReached(int tkt){
   //se la funzione è disattivata, esco prima di iniziare
   if (enableSuperSL == false) return false;
   ...
   if (OrderSelect(tkt, SELECT_BY_TICKET)==true) 
   {
      ...
      {
         //superSL = iCustom(nomIndice,0,"Y3_SuperSL",7,0,0.3,superSL_ratio,3,0);
         if (MarketInfo(nomIndice,MODE_BID) < superSL_StopBuy[0]) return true;
      }
      
      if ((OrderType() == OP_SELL) ) // sell order
      {
         //superSL = iCustom(nomIndice,0,"Y3_SuperSL",7,0,0.3,superSL_ratio,4,0);
         if (MarketInfo(nomIndice,MODE_BID) > superSL_StopSell[0]) return true;
      }
   }  
   ...
   if (ArraySize(historicPipsMA) > 0) cLib = historicPipsMA[ArraySize(historicPipsMA)-1];
    ...
    Comment( "\n ","\n ","\n ","\n ",
           ...
           // "\n Orari: ",openHours,
            ...
            "\n SELL Conditions  : 0.",sellConditions[0]," 1.",sellConditions[1]," 2.",sellConditions[2]," 3.",sellConditions[3]," 4.",sellConditions[4]," 5.",sellConditions[5]," 6.",sellConditions[6]," 7.",sellConditions[7]," 8.",buyConditions[8],
            "\n +-----------------------------   ",
            "\n Time: ",TimeCurrent(),
