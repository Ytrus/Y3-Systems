...
void OnTick()
{
   ...
   if ( (moreThanOneOrderOpened()) && (cumulativeProfitIsPositive())                   // se la somma dei profitti è positiva
      ) closeAllOrders();
...
//---
   ...
   */
   ...
   for(int pos=0;pos<OrdersTotal();pos++)
   {
      ...
      if ( 
         ( (onlyOneOrder()) && (stoppedBySupertrend(OP_BUY)) && (OrderProfit() > 0) )                 // se il superTrenbd si gira ed il profitto di questa posizione è positivo
...
*/
   ...
   for(int pos=0;pos<OrdersTotal();pos++)
   {
      ...
      // chiusura SINGOLO ordine 
      if ( 
         ((onlyOneOrder()) && (stoppedBySupertrend(OP_SELL)) && (OrderProfit() > 0)  )                // se il superTrenbd si gira ed il profitto di questa posizione è positivo
         //|| (protector(OrderTicket(), protectionStartDistance, protectionCloseDistance))            // se ha raggiunto una certa percentuale di profitto e poi torna indietro
      ) closeOrder(OrderTicket());
...
//--------------------------------------------------------+
bool closeAllOrders(int ot = 0){ //ot adesso è inutile
   int total = OrdersTotal();
   double price = MarketInfo(nomIndice,MODE_BID);
   color c = clrGreen;
   bool result = false;
...
//-----------------------------------------------------------------+
//             La somma dei profitti è positiva? 
//-----------------------------------------------------------------+
