...
//--------------------------+
string bot_name = "Random 0.1.1 Bar.Prec"; // Per il TP e lo SL uso l'ATR orario, poi muovo lo SL al salire del profitto
...
{ 
   ...
   if (iBarShift(nomIndice,0,lastAnalizedBarTime,false) > 0 ) //(Volume[0] == 1)
   {
      ...
      rand_number+=MathRand()%6; // number from 0 to 5
...
//-----------------enter buy order---------------------------+
   ...
   if (!existOrderOnThisBar(0))                                buyConditions[0] = true;   // se NON ho un ordine già aperto in questa barra (apre un solo ordine per ogni direzione)
   if (enabledHours[Hour()] == true)                           buyConditions[1] = true;   // A questa ora posso tradare
   if (!existOpendedAndClosedOnThisBar(1))                     buyConditions[2] = true;   // Se non ho 1 ordine aperto e chiuso in questa barra
   if (Close[1] < Open[1])                                     buyConditions[3] = true;   // la bara precedente era BULL
...
//-----------------enter sell order----------------------------+
   ...
   if (!existOrderOnThisBar(1))                                sellConditions[0] = true;     // se NON ho un ordine già aperto in questa barra (apre più ordini in ogni direzione)
   if (enabledHours[Hour()] == true)                           sellConditions[1] = true;     // A questa ora posso tradare
   if (!existOpendedAndClosedOnThisBar(1))                     sellConditions[2] = true;     // Se non ho 1 ordine aperto e chiuso in questa barra   
   if (Close[1] > Open[1])                                     sellConditions[3] = true;     // La barra precedente era BEAR
...
bool slReached(int tkt)
{
   ...
   //double spread = MarketInfo(nomIndice,MODE_SPREAD) * Point;
