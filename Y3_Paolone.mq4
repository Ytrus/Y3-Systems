...
bool closeOrder(int tkt){
   ...
   if(!OrderClose(tkt,OrderLots(),price,0,c)) {
      Print("Errore nella chiusura di un ordine: "+(string)GetLastError()); return false;
   }
   else{
      saveOrderData(tkt,OrderLots(),OrderProfit());
   }
...
double getDistance(int shift){
   ...
   //result = NormalizeDouble((maxDistance/10)*9,Digits);  //original
   result = NormalizeDouble(maxDistance*distanceMultiplier,Digits);
...
// ==================================
//      log csv di un ordine 
// ==================================
bool saveOrderData(int tkt, double lots, double profit){
   string fileName = "Yoyo_"+nomIndice+".csv";
   if(IsTesting()) fileName = "Yoyo_"+nomIndice+"_Testing.csv";
   ...
   if(file_handle!=INVALID_HANDLE){
      if (FileSeek(file_handle,0,SEEK_END)==true) {
         ...
         FileWrite(file_handle,tkt,lots,profit);
      }
      FileClose(file_handle);
   }
   else PrintFormat("Errore nell'apertura del file %s: Error code = %d",fileName,GetLastError());   
...
int deleteOrderData()
{
   string fileName = "Yoyo_"+nomIndice+".csv";
   if(IsTesting()) fileName = "Yoyo_"+nomIndice+"_Testing.csv";
   if (!FileDelete(fileName)) Print("File ",fileName," assente o protetto");
