...
//--------------------------+
string bot_name = "HA Mago 0.3.31"; // versione base con protector
...
//-----------------enter buy order---------------------------+
   ...
   if ((haCloseM15[1] > haOpenM15[1]) && (haCloseM15[2] < haOpenM15[2]))                           buyConditions[0] = true;   // HA precedente nei 15M è bull e quella prima è bear
   if ( ((haClose[0] < haOpen[0]) && (haClose[1] < haOpen[1])) || ((haClose[1] < haOpen[1]) && (haClose[2] < haOpen[2])) )   buyConditions[1] = true;   // Questa barra e la precedente oppure le due precedenti sono BEAR
...
//-----------------enter sell order----------------------------+
   ...
   if ((haCloseM15[1] < haOpenM15[1]) && (haCloseM15[2] > haOpenM15[2]))                           sellConditions[0] = true;  // HA precedente nei 15M è bull e quella prima è bear
   if ( ((haClose[0] > haOpen[0]) && (haClose[1] > haOpen[1])) || ((haClose[1] > haOpen[1]) && (haClose[2] > haOpen[2])) )   sellConditions[1] = true;  // Questa barra e la precedente sono BULL oppure le due precedenti sono BULL
...

{
   ...
   for (int orx=1;orx<=numberOfOrders;orx++)
   {
      ...
      {  
         ...
         stoploss   = (MathMin(Low[0],Low[1]) - (SL_added_pips*Point));
...

{
   ...
   for (int orx=1;orx<=numberOfOrders;orx++)
   {
   ...
   
      {
         ...
         stoploss   = (MathMax(High[0],High[1]) + (SL_added_pips*Point));
